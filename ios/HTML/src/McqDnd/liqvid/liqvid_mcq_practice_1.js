...
//        this.dd_sound.load();
        ...
        this.updateProgress = function() {
            ...
            if(this.questionTaken < this.totalNumberOfQuestions) 
        ...
        this.renderDragAndDropQuestion= function(question) {
            ...
            /* Now add question div*/
            this.qaArea.questionDiv = createDiv("qadiv_question");
            this.qaArea.touchDDAnswer = createDiv("qadiv_SPanswer ");
            ...
            this.currentOptionsArray = new Array();
            for(var i=0; i< question.answers.answer.length; i++) {
                var opt = createLi("drag_item_circle");
                $(opt).html(question.answers.answer[i].content.text);
                $(opt).click(handleDragAndDropToggle);
                opt.root = this;
                opt.stat =1 ;
                $(this.qaArea.optionDiv).append(opt);
                opt.answer_position=question.answers.answer[i].position.text;
            }
        ...
        this.clearDDOptions = function() {
            for(var i=0;i< this.orderedOptionArray.length; i++) {
                ...
                this.orderedOptionArray[i].className = "drag_item_circle";
        ...
        this.toggleDDOption = function(opt) {
            if(opt.stat==1) {
                ...
                opt.stat = 1;
                opt.className= "drag_item_circle";
                $(opt).show();
                var idx_to_remove = -1;
                for(var i=0;i< this.orderedOptionArray.length; i++) {
        ...
        this.renderDragAndDropAnswer = function() {
            ...
            for(var i=0;i< this.orderedOptionArray.length; i++) {
                var dropItem = createDiv("drag_item_circle drag_item_circle");
        ...
        this.showAnswerScreen = function(isCorrect, answerText) {
            ...
            var pctage = (100*(this.correctQuestions / this.totalNumberOfQuestions));
            pctage |= 0;
            ...
            progHtml += "<p>"+pctage+"/100</p></div></div>";
