...
- (void)drawText:(CGRect)rect context:(CGContextRef)c{
  ...
  // NSString *formatString = [NSString stringWithFormat:@"%%.%dd", (int)self.decimalPlaces];
    NSString *formatString = @"";
    if ([self.unitString rangeOfString:@"mins completed"].location != NSNotFound) {
        formatString = @"  %02d";
    }
    else
    {
  ...
  if (self.countdown) {
    textToPresent = [NSString stringWithFormat:formatString, (int)(self.maxValue - self.value)];
  } else {
    textToPresent = [NSString stringWithFormat:formatString, (int)self.value];
  }
  ...
  CGPoint textCenter = CGPointMake(
    ...
    CGRectGetMidY(rect)-percentSize.height/2 + self.textOffset.y-updown
  );
