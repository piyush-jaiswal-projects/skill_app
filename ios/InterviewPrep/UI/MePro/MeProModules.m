...
@interface MeProModules ()<UITableViewDelegate,UITableViewDataSource>
{
    UIView * bar;
...
- (void)viewDidLoad {
    ...
    navigation_screen_title= [[UILabel alloc]initWithFrame:CGRectMake(0, 12, navigation_screen_title_bar.frame.size.width-30, 20)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row == 0)
    {
        ...
        UILabel *learntestScoreIns = [[UILabel alloc]initWithFrame:CGRectMake(10,5,LearningCurve.frame.size.width, 15)];
        ...
        [learntestScoreIns setTextColor:[self getUIColorObjectFromHexString:@"#ffffff" alpha:1.0]];
         ...
         NSString * Searchstr = [[NSString alloc]initWithFormat:@"%@ %@:",LEVEL_TEXT,self.GSE_level];
        
        NSString * str = [[NSString alloc]initWithFormat:@"%@ %@: Learning Modules",LEVEL_TEXT,self.GSE_level ];
        ...
        [timeString addAttribute:NSForegroundColorAttributeName value:[self getUIColorObjectFromHexString:@"#ffffff" alpha:1.0] range:range];
        [timeString addAttribute:NSFontAttributeName value:[UIFont boldSystemFontOfSize:14] range:range];
    ...
    else
    {
        ...
        else
        {
            ...
            if(second < [modulesDataArr count] ){
                ...
                if([[obj2 valueForKey:DATABASE_CATLOGCONT_ASSESSMENT_TYPE]intValue] == 2  )
                {
                    ...
                    title2.frame = CGRectMake(10, 20, rightView.frame.size.width-20, 100);
        ...
        else
        {
            ...
            if([[jsonResponse1 valueForKey:@"action"] isEqualToString:UPDATEACTION])
            {
                ...
                UIAlertAction* YesAction = [UIAlertAction actionWithTitle:[appDelegate.langObj get:@"UPDATE_NOW" alter:@"Update Now"] style:UIAlertActionStyleCancel
                    ...
                    [self addProcessInQueue:jsonResponse1 :@"assessmentUpdate":@"MeProModules"];
                }];
            ...
            else
            {
                if(![appDelegate checkZipPath:zipName])
                {
                    ...
                    UIAlertAction* YesAction = [UIAlertAction actionWithTitle:[appDelegate.langObj get:@"DOWNLOAD_NOW" alter:@"Download Now "]  style:UIAlertActionStyleCancel
                                                                      handler:^(UIAlertAction * action) {
                        [self addProcessInQueue:jsonResponse1 :@"assessmentDownload":@"MeProModules"];
                    }];
...
//            [baseCircleLine addSubview:level_lbl];
//
//            if([self.GSE_level intValue] > [[[levelDataArr objectAtIndex:i]valueForKey:@"level"]intValue])
//            {
//                baseline.backgroundColor = [self getUIColorObjectFromHexString:@"#d2eae4" alpha:1.0];
...
//
//            }
//            else if([self.GSE_level intValue] == [[[levelDataArr objectAtIndex:i]valueForKey:@"level"]intValue])
//            {
//                baseline.backgroundColor = [self getUIColorObjectFromHexString:@"#d2eae4" alpha:1.0];
//                backGbaseline.backgroundColor = [self getUIColorObjectFromHexString:@"#f5f5f5" alpha:1.0];
//                baseCircleLine.backgroundColor = [UIColor whiteColor];
...
-(void)loadNewLevel :(id)sender
{
    ...
    if((![appDelegate.engineObj isCourseExist:[appDelegate.workingCourseObj valueForKey:DATABASE_COURSE_CEDGE]]) || ([[appDelegate.workingCourseObj valueForKey:@"action"]integerValue] == 2))
    {
        ...
        if(([[appDelegate.workingCourseObj valueForKey:@"action"]integerValue] == 2))
        {
            ...
            [self addProcessInQueue:appDelegate.workingCourseObj:@"courseUpdate":@"MeProModules"];
        }
        else
        {
            [self addProcessInQueue:appDelegate.workingCourseObj :@"courseDownload" :@"MeProModules"];
        }
    }
    else
    {
...
- (void)readServerResponse:(NSNotification *) notification
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
       if([[[notification object]valueForKey:NETWORKSTATUS] intValue] == [SUCCESSRESULT intValue] && [[[notification object]valueForKey:@"SERVICE"] isEqualToString:SERVICE_ASSESSMENTQUIZDATA])
        {
